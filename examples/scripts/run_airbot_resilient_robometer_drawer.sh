#!/bin/bash
# Crash-resilient DSRL Airbot training WITH Robometer dense rewards (open_drawer).
#
# Difference vs run_airbot_resilient_rm_drawer.sh:
...
#     Note this replaces the milestone-based DrawerRewardModel, so unlike
#     --rm_variant drawer the operator's 1/0 label is applied by the caller
...
#
# Reward resolution: 175/25 = 7 query steps vs Robometer's max_frames=8 progress
# points per pass — ~1:1, no meaningful interpolation loss.
#
...
#
# Behavior:
#   - Same outer bash while-true restart loop as run_airbot_resilient_drawer.sh.
...
# ============================================================
# Fill in your configuration below (must match run_airbot_resilient_drawer.sh)
# ============================================================
