...
public class HeadsetPublisher : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Enable input actions (Quest controllers + headset)
        headsetPose.action.Enable();
        headsetRotation.action.Enable();

        handPoseLeft.action.Enable();
        handRotationLeft.action.Enable();

        handPoseRight.action.Enable();
        handRotationRight.action.Enable();

        // If headsetPose is reading null, Debug log an error
        if (headsetPose == null || headsetRotation == null ||
            handPoseLeft == null || handRotationLeft == null ||
            handPoseRight == null || handRotationRight == null)
        {
            Debug.LogError("HeadsetPublisher: One or more InputActionReferences are not assigned.");
        }
    ...
    void Update()
    {
        ...
        var headPos = headsetPose.action.ReadValue<Vector3>();
        var leftPos = handPoseLeft.action.ReadValue<Vector3>();
        var rightPos = handPoseRight.action.ReadValue<Vector3>();

        if (headPos == Vector3.zero && leftPos == Vector3.zero && rightPos == Vector3.zero)
        {
            // Only log occasionally to avoid spam
            if (Time.frameCount % 120 == 0)
                Debug.LogWarning("HeadsetPublisher: XR pose inputs are all zero. Actions may be unbound or tracking not active.");
        }
